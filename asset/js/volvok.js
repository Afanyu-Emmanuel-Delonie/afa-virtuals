class VolkovChat {
    constructor() {
        this.session = {
            askedKeywords: [],
            popupSeen: this.hasSeenPopupThisSession(),
            chatStarted: false
        };
    ...
    hasSeenPopupThisSession() {
        try {
            // Check if popup was seen in this browser session
            return sessionStorage.getItem('volkovChatPopupSeen') === 'true';
        } catch (e) {
    ...
    markPopupAsSeen() {
        try {
            ...
            sessionStorage.setItem('volkovChatPopupSeen', 'true');
        } catch (e) {
            // Fallback if sessionStorage is not available - continue normally
            console.log('SessionStorage not available');
        }
    }

    init() {
        // Show popup after 10 seconds if not seen in this session
        setTimeout(() => {
    ...
    async processMessage(message) {
        ...
        const greetingMatch = this.findBestMatch(message);
        if (greetingMatch && greetingMatch.priority === 11) {
